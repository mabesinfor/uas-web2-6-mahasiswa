...
const handleDelete = (id) => {
  if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
    router.delete(route('mahasiswa.destroy', { mahasiswa: id }));
  }
};
...
const handleBulkDelete = () => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data yang dipilih?')) {
        ...
        form.delete(route('mahasiswa.bulk-destroy'), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                selectedItems.value = [];
...
const toggleSelectAll = (event) => {
    if (event.target.checked) {
        selectedItems.value = props.mahasiswa.map(mhs => mhs.id);
    } else {
        selectedItems.value = [];
    }
};
...
const isAllSelected = computed(() => {
    return selectedItems.value.length === props.mahasiswa.length;
});
...
const previousPage = () => {
    if (props.pagination.prev_page_url) {
        router.get(route('mahasiswa.index', { search: search.value, page: props.pagination.current_page - 1 }));
    }
};
...
const goToPage = (page) => {
    router.get(route('mahasiswa.index', { search: search.value, page: page }));
};
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Mahasiswa</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between mb-2">
                            <div class="space-x-2">
                                <Link :href="route('mahasiswa.create')" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        ...
                        <div v-if="props.mahasiswa.length === 0" class="text-center py-4 text-gray-600">
                        ...
                        <table v-else class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr>
                                    ...
                                    <th @click="handleSort('nama')" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                        Nama 
                                        <span class="ml-1">{{ getSortIcon('nama') }}</span>
                                    </th>
                                    <th @click="handleSort('nim')" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                        NIM 
                                        <span class="ml-1">{{ getSortIcon('nim') }}</span>
                                    </th>
                                    <th @click="handleSort('program_studi_id')" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                        Program Studi
                                        <span class="ml-1">{{ getSortIcon('program_studi_id') }}</span>
                                    </th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="mhs in props.mahasiswa" :key="mhs.id">
                                    <td class="px-6 py-4 whitespace-no-wrap">
                                        <input class="rounded-[4px]" type="checkbox" v-model="selectedItems" :value="mhs.id">
                                    </td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mhs.nama }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mhs.nim }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mhs.program_studi.nama }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap flex gap-2">
                                        <Link :href="route('mahasiswa.edit', { mahasiswa: mhs.id })" class="text-yellow-600 hover:text-yellow-900">Edit</Link>
                                        <button @click="handleDelete(mhs.id)" class="text-red-600 hover:text-red-900">Hapus</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
