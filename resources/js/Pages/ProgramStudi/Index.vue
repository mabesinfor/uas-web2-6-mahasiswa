...
const handleDelete = (id) => {
  if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
    router.delete(route('program-studi.destroy', { program_studi: id }));
  }
};
...
const handleBulkDelete = () => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data yang dipilih?')) {
        ...
        form.delete(route('program-studi.bulk-destroy'), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                selectedItems.value = [];
...
const toggleSelectAll = (event) => {
    if (event.target.checked) {
        selectedItems.value = props.programStudi.map(program_studi => program_studi.id);
    } else {
        selectedItems.value = [];
    }
};
...
const isAllSelected = computed(() => {
    return selectedItems.value.length === props.programStudi.length;
});
...
const previousPage = () => {
    if (props.pagination.prev_page_url) {
        router.get(route('program-studi.index', { search: search.value, page: props.pagination.current_page - 1 }));
    }
};
...
const goToPage = (page) => {
    router.get(route('program-studi.index', { search: search.value, page: page }));
};
...
const performSearch = () => {
    ...
    searchTimer = setTimeout(() => {
        router.get(route('program-studi.index', { 
            search: search.value, 
            page: 1,
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Program Studi</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between mb-2">
                            <div class="space-x-2">
                                <Link :href="route('program-studi.create')" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    <span class="hidden sm:inline">Tambah Program Studi</span>
                        ...
                        <div v-if="props.programStudi.length === 0" class="text-center py-4 text-gray-600">
                        ...
                        <table v-else class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="program_studi in props.programStudi" :key="program_studi.id">
                                    <td class="px-6 py-4 whitespace-no-wrap">
                                        <input class="rounded-[4px]" type="checkbox" v-model="selectedItems" :value="program_studi.id">
                                    </td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ program_studi.nama }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap flex gap-2">
                                        <Link :href="route('program-studi.edit', { program_studi: program_studi.id })" class="text-yellow-600 hover:text-yellow-900">Edit</Link>
                                        <button @click="handleDelete(program_studi.id)" class="text-red-600 hover:text-red-900">Hapus</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
