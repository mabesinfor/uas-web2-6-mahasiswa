...
const handleDelete = (id) => {
  if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
    router.delete(route('mata-kuliah.destroy', { mata_kuliah: id }));
  }
};
...
const handleBulkDelete = () => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data yang dipilih?')) {
        ...
        form.delete(route('mata-kuliah.bulk-destroy'), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                selectedItems.value = [];
...
const toggleSelectAll = (event) => {
    if (event.target.checked) {
        selectedItems.value = props.mataKuliah.map(mata_kuliah => mata_kuliah.id);
    } else {
        selectedItems.value = [];
    }
};
...
const isAllSelected = computed(() => {
    return selectedItems.value.length === props.mataKuliah.length;
});
...
const previousPage = () => {
    if (props.pagination.prev_page_url) {
        router.get(route('mata-kuliah.index', { search: search.value, page: props.pagination.current_page - 1 }));
    }
};
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between mb-2">
                            <div class="space-x-2">
                                <Link :href="route('mata-kuliah.create')" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    <span class="hidden sm:inline">Tambah Mata Kuliah</span>
                        ...
                        <div v-if="props.mataKuliah.length === 0" class="text-center py-4 text-gray-600">
                        ...
                        <table v-else class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr>
                                    ...
                                    <th @click="handleSort('kode')" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                        Kode 
                                        <span class="ml-1">{{ getSortIcon('kode') }}</span>
                                    </th>
                                    ...
                                    <th @click="handleSort('sks')" class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                        SKS 
                                        <span class="ml-1">{{ getSortIcon('sks') }}</span>
                                    </th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="mata_kuliah in props.mataKuliah" :key="mata_kuliah.id">
                                    <td class="px-6 py-4 whitespace-no-wrap">
                                        <input class="rounded-[4px]" type="checkbox" v-model="selectedItems" :value="mata_kuliah.id">
                                    </td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mata_kuliah.kode }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mata_kuliah.nama }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ mata_kuliah.sks }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap flex gap-2">
                                        <Link :href="route('mata-kuliah.edit', { mata_kuliah: mata_kuliah.id })" class="text-yellow-600 hover:text-yellow-900">Edit</Link>
                                        <button @click="handleDelete(mata_kuliah.id)" class="text-red-600 hover:text-red-900">Hapus</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
